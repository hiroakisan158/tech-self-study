...
- `useState(0)` は「初期値 0 の記憶箱を貸してください」という React への申請です。
  戻り値は **[現在の値, 更新関数] のタプル**([TS 第 3 章](../../04-typescript-fable-101/chapters/03_objects_arrays.md))で、
  分割代入で受けるのが作法です。名前は `[x, setX]` のペアにする慣習です
- 型は初期値から[推論](../../04-typescript-fable-101/chapters/01_variables.md)されます
...
> 印刷された値(スナップショット)** です。[クロージャ](../../04-typescript-fable-101/chapters/09_array_methods.md)が
...
[型の二重管理](../../04-typescript-fable-101/chapters/13_advanced_types.md)と同じ構図の罠です。
**state は最小限、残りは計算で導く。**
...
> [`this` の束縛問題](../../04-typescript-fable-101/chapters/10_this.md)との果てしない戦い——
