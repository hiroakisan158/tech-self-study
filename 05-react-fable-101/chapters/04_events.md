...
[TypeScript 第 4 章「関数は値である」](../../04-typescript-fable-101/chapters/04_functions.md)と
[第 11 章「イベントループ」](../../04-typescript-fable-101/chapters/11_event_loop.md)の知識が
まるごと流れ込んでいます。
...
- これは [TS 第 11 章の `setTimeout(fn, ...)`](../../04-typescript-fable-101/chapters/11_event_loop.md) と
...
- 型は `React.ChangeEvent<要素の型>` のように書きます。**覚える必要はありません**——
  `onChange={(e) => ...}` とインラインで書けば、[文脈から推論](../../04-typescript-fable-101/chapters/04_functions.md)
...
> ([TS 第 9 章](../../04-typescript-fable-101/chapters/09_array_methods.md))。上の `() => reserve(show.id)` が
