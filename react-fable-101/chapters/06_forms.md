...
[「フォーム入力は必ず文字列」は 1995 年からの Web の掟](../../typescript-fable-101/chapters/02_numbers_strings.md)
でした。`Number(...)` での変換を忘れると、`tickets + 1` が `"11"` になる
[あの `+` の罠](../../typescript-fable-101/chapters/02_numbers_strings.md)を React の中で
...
「予約完了メッセージがまだ無い」状態を、[null を含む union](../../typescript-fable-101/chapters/05_unions.md)
...
「ありえない画面の組み合わせを型で消す」——[TS 第 5 章の判別可能 union](../../typescript-fable-101/chapters/05_unions.md)
...
`push` してはいけない——[TS 第 9 章で予告した「React では複製が前提」](../../typescript-fable-101/chapters/09_array_methods.md)
