...
- 文字列以外の値は `{}` で渡します: `seats={12}`、`isFinale={true}`(値なしの `isFinale` は
  `true` の省略記法)
- props の型は [interface](../../typescript-fable-101/chapters/03_objects_arrays.md) で定義します。
...
[readonly](../../typescript-fable-101/chapters/03_objects_arrays.md) を付ける流儀もありますが、
実は分割代入で受けている限り書き換えても親には影響しません
([オブジェクトの参照](../../typescript-fable-101/chapters/03_objects_arrays.md)のコピーを
...
> ```
>
> [「関数もオブジェクトもすべて値」という JS の性質](../../typescript-fable-101/chapters/04_functions.md)の上に、
...
([Go が継承を持たず合成で設計する](../../go-fable-101/language-overview/README.md)のと同じ時代精神です)。
