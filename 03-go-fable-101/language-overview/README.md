...
隣のフォルダの Python(`02-python-fable-101/language-overview/`)は、Go の弱点(試行錯誤の速さ・表現力・ライブラリ生態系)をちょうど裏返したような言語です。実務では「探索・分析は Python、本番サービスとツールは Go」のような使い分けもよく見られます。読み比べると、それぞれの設計判断がより立体的に見えてきます。
