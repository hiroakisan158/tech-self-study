...
`MenuItemPage` は `async` 関数でした。[React 教材](../../react-fable-101/README.md)では
一度も出てこなかった書き方です。コンポーネントの中で `await` ができるなら、
[第 14 章であれほど苦労した fetch の 3 局面管理](../../react-fable-101/chapters/14_data_fetching.md)は
どこへ行ったのでしょう?
...
[React 第 10 章](../../react-fable-101/chapters/10_rendering.md)で「レンダリングとは、
...
サーバーで動くということは、[TS 教材後半で使った Node.js の全能力](../../typescript-fable-101/chapters/14_runtime_validation.md)が
...
このコードの何が革命なのか、[React 第 14 章の劇評コーナー](../../react-fable-101/chapters/14_data_fetching.md)と
並べると一目瞭然です:
...
> 呼ばれる「レンダリング済みの設計図データ」です([React 第 1 章](../../react-fable-101/chapters/01_jsx.md)で
...
- **`useState` / `useEffect` などの状態系・副作用系フック** — 状態とは
  [「再上演をまたぐ記憶」](../../react-fable-101/chapters/05_state.md)でしたが、サーバーでの
...
> 学習コストの増大やエコシステムの分断([react の overview 6.1/6.5](../../react-fable-101/language-overview/README.md))を
...
3. `data/reviews.json` の 1 件の `stars` を `"5"`(文字列)にして、zod の門番が実行時に止めることを確認してください。[React 第 14 章の演習](../../react-fable-101/chapters/14_data_fetching.md)と同じ実験を、今日は厨房側でやっている——実行場所が変わっても門番の作法は同じです。
