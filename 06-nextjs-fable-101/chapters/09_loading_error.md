...
[React 教材の劇評コーナー](../../05-react-fable-101/chapters/14_data_fetching.md)では、
`loading / success / error` の 3 局面を [union 型](../../04-typescript-fable-101/chapters/05_unions.md) +
...
4. `error.tsx` の `reset` ボタンを連打して、成功するまで再試行できることを確認してください(2 割の失敗率なので数回で成功します)。[React 第 14 章で自作した再試行ボタン](../../05-react-fable-101/chapters/14_data_fetching.md)と比べ、自分で書かなくてよくなったものを列挙してみましょう。
