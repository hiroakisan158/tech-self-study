...
各 `page.tsx` に `<NavBar />` をコピペする——のは、[React 第 2 章で学んだ合成](../../05-react-fable-101/chapters/02_props.md)の
...
仕組みは [React 第 2 章の children](../../05-react-fable-101/chapters/02_props.md) **そのもの** です。
...
> **state も維持されます**([React 第 10 章の「同じ位置・同じ type なら同一人物」](../../05-react-fable-101/chapters/10_rendering.md)が、
> ページ遷移をまたいで働くということです)。
>
...
- [React 第 9 章では `document.title` を useEffect で書き換えました](../../05-react-fable-101/chapters/09_effects.md)が、
...
💡 `export const metadata` は **型注釈 `Metadata` を付ける**([TS 第 1 章](../../04-typescript-fable-101/chapters/01_variables.md))ことで、
...
2. layout の state 維持を体感する実験: `NavBar` に [useState のカウンタ](../../05-react-fable-101/chapters/05_state.md)(押すと増えるだけのボタン)を仮置きし、ページを行き来してもカウントが消えないことを確認してください(確認後は削除)。`<a>` タグ遷移だとどうなるかも比較を。
