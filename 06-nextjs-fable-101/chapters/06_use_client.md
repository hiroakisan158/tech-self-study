...
数量は [state](../../05-react-fable-101/chapters/05_state.md)、ボタンは
[onClick](../../05-react-fable-101/chapters/04_events.md)——2F でさんざん作った UI です。
...
(シリアライズ)** されます。[JSON にできる値](../../04-typescript-fable-101/chapters/03_objects_arrays.md)は
渡せますが、**関数は渡せません**:
...
[React 教材では「props down, events up」のためにコールバックを渡し放題](../../05-react-fable-101/chapters/04_events.md)でした。
あれは全部品が同じ場所(ブラウザ)にいたからです。厨房と客席は **別のマシン** ——
関数(コードとクロージャ)を JSON にして送ることはできません
([TS 第 14 章: JSON に載るのはデータだけ](../../04-typescript-fable-101/chapters/14_runtime_validation.md))。

| 境界を越えられる | 越えられない |
|---|---|
| string / number / boolean / null | **関数**(イベントハンドラ) |
| 配列・プレーンなオブジェクト | クラスのインスタンス([Adventurer](../../04-typescript-fable-101/chapters/07_classes.md) のような) |
...
[React 第 2 章の children](../../05-react-fable-101/chapters/02_props.md) で解決できます:
...
[「枠と中身の分離」](../../05-react-fable-101/chapters/02_props.md)という 2F の設計パターンが、
