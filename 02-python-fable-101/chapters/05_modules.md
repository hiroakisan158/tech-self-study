...
### 補足: `pip install` で実際に何が起きているか

`pip install rich` のとき、裏側では何がダウンロードされ、何が実行されているのでしょうか。

**配布形式は2種類ある**

| 形式 | 中身 | いつビルドするか |
|---|---|---|
| `sdist`(ソース配布) | ソースコードそのもの | インストール時に手元でビルド |
| `wheel`(ビルド済み配布、`.whl`) | インストールしやすく整えた状態 | 公開時に開発者側で1回だけ |

`pip install` は基本的に `wheel` を探しに行きます。PyPI(パッケージを配布する
サーバー)から自分の環境に合う `.whl` をダウンロードし、中身を
`.venv/lib/.../site-packages/` に展開するだけです。

**ただし「wheel = 機械語バイナリ」ではない**

ここが誤解しやすいところです。`wheel` は「built distribution(ビルド済み配布)」
と呼ばれますが、この「built」は「機械語にコンパイル済み」という意味とは限りません。

- **純粋な Python パッケージ**(`rich` など、`.py` だけで書かれたもの): `wheel` を
  展開しても中身は `.py` の**ソースコードそのもの**です。「built」の意味は
  「`setup.py` を実行するビルド手順を省略できるよう、あらかじめ展開しやすい形に
  整えておいた」というだけで、コンパイルは一切発生しません。従来通り
  インタプリタがソースを読んで実行します
- **C 拡張を含むパッケージ**(`numpy` や `pandas` など、内部を C/C++ で書いている
  もの): こちらは `wheel` の中に **本物の機械語バイナリ**(`.so` / `.pyd`)が
  含まれます。C 拡張のコンパイルは PyPI へ公開する開発者・CI 側が済ませており、
  ファイル名の `cp311-manylinux_x86_64` のような部分が「CPython 3.11 用・
  Linux x86_64 用にコンパイル済み」を表す **ABI タグ** です。あなたの環境に
  合うタグの wheel が無いときだけ、pip は `sdist` を落として手元でビルドしよう
  とします(この時だけ C コンパイラが要求されるのはこのためです)。

まとめると、**「`pip install` で手に入るのは常にソース」でも「常にバイナリ」
でもなく、パッケージの中身次第**です。
